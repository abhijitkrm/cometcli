...
func DoctorCmd(reg *toolkit.Registry) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			// Checks are independent — fan them out in parallel, each into
			// a fixed slot so ordering stays stable and no mutex is needed.
			type spec struct {
				name string
				tool string
				args toolkit.Args
				eval func(*toolkit.Result, error) Check
			}
			var specs []spec
			run := func(name, tool string, args toolkit.Args, eval func(*toolkit.Result, error) Check) {
				specs = append(specs, spec{name, tool, args, eval})
			}
			...
			for i, sp := range specs {
				...
				go func(i int, sp spec) {
					defer wg.Done()
					t, ok := reg.Get(sp.tool)
					if !ok {
						checks[i] = Check{sp.name, false, false, "tool not registered"}
						return
					}
					sub, cancel := toolkit.WithDeadline(c, 30*time.Second)
					res, err := t.Run(sub, sp.args)
					sub.Close()
					cancel()
					chk := sp.eval(res, err)
					chk.Name = sp.name
